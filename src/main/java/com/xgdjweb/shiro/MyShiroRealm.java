package com.xgdjweb.shiro;


import com.xgdjweb.bean.Admin;
import com.xgdjweb.jwt.JWTToken;
...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        String number = JWTUtil.getNumber(principalCollection.toString());
        List<Admin> list = adminService.getAdminByNumber(number);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        System.out.println("————xgdjweb————");
        Admin userInfo = null;
        //获取用户的输入的账号.
        String token = (String) authenticationToken.getCredentials();
        String number = JWTUtil.getNumber(token);

        if (number == null || !JWTUtil.verify(token, number)) {
